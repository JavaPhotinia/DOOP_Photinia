package org.clyze.doop.soot.pointans.analysis;

import org.aspectj.weaver.UnresolvedType;
import org.aspectj.weaver.patterns.*;
import org.clyze.doop.soot.pointans.bean.AOPTargetModel;
import org.clyze.doop.soot.pointans.bean.AspectModel;
import org.clyze.doop.soot.pointans.bean.InsertMethod;
import org.clyze.doop.soot.pointans.enums.AdviceEnum;
import org.clyze.doop.soot.pointans.mock.GenerateSyntheticClass;
import org.clyze.doop.soot.pointans.mock.GenerateSyntheticClassImpl;
import org.clyze.doop.soot.pointans.utils.JimpleUtils;
import soot.*;
...
public class AOPParser {
    ...
    protected SootMethod aroundParser(AspectModel aspectModel, SootMethod targetMethod) {
        ...
        SootMethod newAspectMethod = new SootMethod(aspectModel.getSootMethod().getName()
                + "_" + getHashCode(aspectModel.getSootClass().getName()
    ...
    protected void insertAOPAround(SootMethod currentMethod, SootMethod calleeMethod) {
        ...
        List<Local> paramLocalList = body.getParameterLocals();
        while (paramCount < 0) {
            if (paramLocalList.get(0).getType().toString().contains("JoinPoint")) {
                paramList.add(paramLocalList.get(0));
                paramLocalList.remove(0);
                continue;
            }
            paramLocalList.remove(0);
            paramCount++;
        }
        paramList.addAll(paramLocalList);
