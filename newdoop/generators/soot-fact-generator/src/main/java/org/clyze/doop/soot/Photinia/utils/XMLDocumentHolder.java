...
import org.clyze.doop.soot.Photinia.bean.AopXMLResultBean;
...
public class XMLDocumentHolder {
    ...
    private final List<ConstructorArgBean> argConstructors = new ArrayList<>();
    private final Map<String, String> filter = new HashMap<>();
    private final Map<String, String> shiro = new HashMap<>();
    ...
    public void hasFilter(Document doc) {
        HashMap<String, String> filterNameMap = new HashMap<>();
        List<Element> eles = doc.getRootElement().elements("filter");
        for (Element ele : eles) {
            String filterName = "";
            String filterClass = "";
            if (ele.element("filter-name") != null) {
                filterName = ele.element("filter-name").getText();
            }
            if (ele.element("filter-class") != null) {
                filterClass = ele.element("filter-class").getText();
            }
            filterNameMap.put(filterName, filterClass);
        }

        List<Element> mappingEles = doc.getRootElement().elements("filter-mapping");
        for (Element ele : mappingEles) {
            String filterName = "";
            String urlPattern = "";
            if (ele.element("filter-name") != null) {
                filterName = ele.element("filter-name").getText();
            }
            if (ele.element("url-pattern") != null) {
                urlPattern = ele.element("url-pattern").getText();
            }
            filter.put(filterNameMap.get(filterName), urlPattern);
        }
    }

    public void hasShiroConfig(Document doc) {
        HashMap<String, String> shiroNameMap = new HashMap<>();
        if (this.elements.containsKey("shiroFilter")) {
            Element shiroFilter = this.elements.get("shiroFilter");
            for (Element property : shiroFilter.elements("property")) {
                if (property.attribute("name").getValue().equals("filters")) {
                    for (Element map : property.elements("map")) {
                        for (Element entry : map.elements("entry")) {
                            String key = entry.attribute("key").getText();
                            String valueRef = entry.attribute("value-ref").getText();
                            if (this.elements.containsKey(valueRef)) {
                                Element e = this.elements.get(valueRef);
                                valueRef = e.attributeValue("class");
                            } else {
                                valueRef = "bean:" + valueRef;
                            }
                            shiroNameMap.put(key, valueRef);
                        }
                    }
                }
            }
        ...
        if (this.elements.containsKey("shiroFilterChainDefinitions")) {
            Element constructor = this.elements.get("shiroFilterChainDefinitions").element("constructor-arg");
            String[] shiroPaths = constructor.element("value").getText().split("\n");
            for (String shiroPath : shiroPaths) {
                String shiroPathClean = shiroPath.replaceAll("\\s+", "");
                if (shiroPathClean.equals("")) {
                    continue;
                }
                String[] pathAndAuth = shiroPathClean.split("=");
                String pathKey = pathAndAuth[0];
                String authValue = pathAndAuth[1];
                shiro.put(pathKey, shiroNameMap.getOrDefault(authValue, authValue));
            }
        }
    }
    ...
    public Map<String, String> getShiroMap() {
        return shiro;
    }
